...
def run_robust_loss_research(
    paths: ProjectPaths,
    config_name: str = "model_huber_research",
) -> dict[str, Any]:
    research_config = load_config(paths, config_name)
    model_config = load_config(paths, "model")
    objective = dict(research_config["lightgbm_regression_objective"])
    model_config["lightgbm_regression_objective"] = objective
    ...
    if not splits:
        raise RuntimeError("No development folds matched the robust-loss period")
    ...
    report_path = immutable_write_json(
        paths.reports / "research" / "model-huber-development.json",
        report,
    )
...
def main() -> None:
    ...
    parser.add_argument("--config", default="model_huber_research")
    args = parser.parse_args()
    report = run_robust_loss_research(
