...
def run_recent_window_research(
    paths: ProjectPaths,
    config_name: str = "model_recent_window_research",
) -> dict[str, Any]:
    research_config = load_config(paths, config_name)
    model_config = load_config(paths, "model")
    model_config["training_window_years"] = int(
        research_config["training_window_years"]
    )
    ...
    if not splits:
        raise RuntimeError("No development folds matched the recent-window period")
    ...
    ][["symbol", "trade_date", "label_regression"]].copy()
    baseline_path = (
        paths.reports
        / "research"
    ...
    report = {
        "status": (
            "RECENT_WINDOW_DEVELOPMENT_ACCEPTED"
            if accepted
            else "RECENT_WINDOW_DEVELOPMENT_REJECTED"
        ),
        "created_at": datetime.now(UTC).isoformat(),
        "development_period": [start.date().isoformat(), end.date().isoformat()],
        "training_window_years": int(research_config["training_window_years"]),
    ...
    report_path = immutable_write_json(
        paths.reports / "research" / "model-recent-window-development.json",
        report,
    )
...
def main() -> None:
    ...
    parser.add_argument("--config", default="model_recent_window_research")
    args = parser.parse_args()
    report = run_recent_window_research(
